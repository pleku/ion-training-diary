(function() {
	...
	window.vaadin = window.vaadin || {
		initApplication: function(appId, config) {
				...
				r.onreadystatechange = function (aEvt) {  
					if (r.readyState == 4) {
						...
						if (r.status == 200){
							...
							// Try bootstrapping again, this time without
							// fetching missing info
							bootstrapApp(false);
						} else {
			...
			if (getConfig("debug")) {
				// TODO debug state is now global for the entire page, but
				// should somehow only be set for the current application
